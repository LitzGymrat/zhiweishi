...
3. `deepseek_v4_flash`：使用 `deepseek-v4-flash`；默认复用项目的 DeepSeek 密钥和地址，也可用 `eval_deepseek_v4_flash_*` 覆盖。
...
上面的命令仅预览：会确认冻结样本数、数据哈希、三个候选接口和 Gemini judge 的可用状态，不会调用外部模型。
...
每次执行保存 `manifest.json`、`cases.jsonl`、`generations.jsonl`、`judgements.jsonl` 与 `summary.json`。禁止利用 test 的 judge 分数反复调 prompt、选 checkpoint 或改训练数据；这类决策应使用 development 集完成。
