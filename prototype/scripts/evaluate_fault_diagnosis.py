...
CANDIDATE_ORDER = ("base", "finetuned", "deepseek_v4_flash")
...
def candidate_endpoints(config: AppConfig) -> list[dict[str, Any]]:
    return [
        ...
        endpoint(
            "deepseek_v4_flash",
            model=config.eval_deepseek_v4_flash_model,
            base_url=config.eval_deepseek_v4_flash_base_url or config.deepseek_base_url,
            api_key=config.eval_deepseek_v4_flash_api_key or config.deepseek_api_key,
        ),
    ]
...
def request_candidate(client: OpenAI, config: dict[str, Any], case: FaultCase, max_tokens: int) -> dict[str, Any]:
    try:
        ...
        try:
            ...
            return {"status": "error", "error": f"JSON 模式失败：{first_error}；普通请求失败：{second_error}"}
    parsed, parse_error = parse_json_object(content)
    return {"status": "ok", "response_text": content, "parsed_output": parsed, "parse_error": parse_error}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="对冻结 test 集的辅助排查任务执行三模型盲评。")
    parser.add_argument("--dataset", type=Path, default=DEFAULT_DATASET)
    parser.add_argument("--output-dir", type=Path)
    parser.add_argument("--workers", type=int, default=4)
    parser.add_argument("--candidate-max-tokens", type=int, default=1600)
    ...
    clients = {candidate_id: OpenAI(api_key=item["api_key"], base_url=item["base_url"]) for candidate_id, item in available.items()}
    ...
    if not judge["available"]:
        ...
        judge_client = OpenAI(api_key=judge["api_key"], base_url=judge["base_url"])
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = {
